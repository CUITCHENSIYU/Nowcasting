...
class ConvGRU(nn.Module):
    ...
    def forward(self, input, hidden_state=None, return_sequence=True):
        """
        Parameters
        ----------
        input_tensor:
        ...
        hidden_state:
        return_sequence:
            If False, skip stacking all timesteps (saves memory when only the last state is needed).
        Returns
        -------
        ...
        for l, (gru_cell, hid_dp) in enumerate(zip(self.cell_list, self.hidden_dps)):
            if isinstance(cur_layer_input, torch.Tensor):
                if cur_layer_input.dim() == 5:
                    steps = [cur_layer_input[i] for i in range(cur_layer_input.size(0))]
                else:
                    steps = [cur_layer_input]
            else:
                steps = list(cur_layer_input)
            ...
            if return_sequence:
                cur_layer_input = torch.stack(output_inner)
                if l != self.n_layers:
                    cur_layer_input = hid_dp(cur_layer_input)
            elif l < self.n_layers - 1:
                cur_layer_input = [hid_dp(h.unsqueeze(0)).squeeze(0)]
